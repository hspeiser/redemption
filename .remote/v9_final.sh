...
sleep 2
best_ck=""; best_n=0
for ck in data/fastsim_runs/ppo_v9rest/finish_*.pt data/fastsim_runs/ppo_v9rest/latest.pt; do
  [ -f "$ck" ] || continue
  n=$(.venv/bin/python scripts/fastsim_eval.py --ckpt "$ck" \
    ...
    --fov-vision --demo-corridor 1.5 --speed-cap 8 --spawn-at-rest --action-smoothness 0.12 --act-delay-min 1 --obstacles data/vq2_obstacles_inflated.json \
...
.venv/bin/python scripts/fastsim_export.py --ckpt "$best_ck" \
  --out /mnt/c/Users/henry/vq2_ppo_v9rest.pt
