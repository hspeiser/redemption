...
sleep 2
best_ck=""; best_n=0
for ck in data/fastsim_runs/ppo_v10hybrid/finish_*.pt data/fastsim_runs/ppo_v10hybrid/latest.pt; do
  [ -f "$ck" ] || continue
  n=$(.venv/bin/python scripts/fastsim_eval.py --ckpt "$ck" \
    ...
    --fov-vision --action-smoothness 0.06 --act-delay-min 1 --spawn-at-rest \
...
.venv/bin/python scripts/fastsim_export.py --ckpt "$best_ck" \
  --out /mnt/c/Users/henry/vq2_ppo_v10hybrid.pt
